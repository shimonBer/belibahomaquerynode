                    ...
                    if(reports && reports.length > 0){
                        ...
                        let trainee_arr = reports.map((report) => {
                            if(report._doc.to && report._doc.from){
                                return `${report._doc.date.toLocaleString("he-IL")}, ${report._doc.studyTime} study hours: ${report._doc.from} - ${report._doc.to}`
                            }else{
                                return `${report._doc.date.toLocaleString("he-IL")}, ${report._doc.studyTime} study hours`;
                            }
                        })
